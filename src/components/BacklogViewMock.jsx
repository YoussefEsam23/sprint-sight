...
import '../styling/BacklogView.css'; // <-- NEW IMPORT
...
const BacklogViewMock = () => {
  ...
  useEffect(() => {
    ...
    setTasks(getLocalTasksDB()[projectId] || []); 
  ...
  return (
    <>
        ...
        <div className="content-left">
          ...
          <div className="story-list">
            {stories.map(story => {
              ...
              return (
                // zIndex is kept inline because it is a dynamic JS state calculation
                <div key={story.id} className="story-card" style={{ zIndex: openMenuId === story.id ? 50 : 1 }} onClick={() => setSelectedStory(story)}>
                  ...
                  <div className="story-info">
                    ...
                    <div className="bv-story-meta">
                      <span>Status: <strong className="bv-badge-highlight" style={{background: 'none', padding: 0}}>{story.status}</strong></span>
                      <span>Priority: <strong>{story.priority}</strong></span>
                      <span className="bv-badge">{storyTaskCount} Tasks</span>
                      {story.sprintId && <span className="bv-badge-highlight">In Sprint</span>}
                    </div>
                  </div>
                  ...
                  <div className="menu-container">
                    ...
                    {openMenuId === story.id && (
                      <div className="dropdown-menu">
                        <button className="dropdown-item" onClick={(e) => { e.stopPropagation(); openStoryModal(story); }}>Edit Story</button>
                        <button className="dropdown-item" style={{color: '#e74c3c'}} onClick={(e) => handleDeleteStory(e, story.id)}>Delete Story</button>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {stories.length === 0 && (
              <div className="bv-empty-state">
                ...
                <p>Click '+ New User Story' to start planning your project.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {selectedStory && (
        <div className="content-left">
           <button className="bv-back-btn" onClick={() => setSelectedStory(null)}>
           ...
           <div className="page-header" style={{ marginBottom: '2rem' }}>
            <div>
              ...
              <p className="bv-task-creator" style={{marginTop: '0.5rem', fontSize: '0.9rem'}}>{selectedStory.description}</p>
            </div>
            <div className="header-actions">
          ...
          <div className="story-list">
            {storyTasks.map(task => (
              <div key={task.id} className="story-card" style={{ zIndex: openMenuId === task.id ? 50 : 1 }}>
                ...
                <div className="story-info">
                  ...
                  <div className="bv-story-meta">
                    <span className="bv-badge">👤 {task.assignedTo}</span>
                    <span>Status: <strong className="bv-badge-highlight" style={{background: 'none', padding: 0}}>{task.status}</strong></span>
                    <span>Priority: <strong>{task.priority}</strong></span>
                  </div>
                  
                  <div className="bv-task-creator">
                ...
                <div className="menu-container">
                  ...
                  {openMenuId === task.id && (
                    <div className="dropdown-menu">
                      <button className="dropdown-item" onClick={(e) => { e.stopPropagation(); openTaskModal(task); }}>Edit Task</button>
                      <button className="dropdown-item" style={{color: '#e74c3c'}} onClick={(e) => handleDeleteTask(e, task.id)}>Delete Task</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            {storyTasks.length === 0 && (
              <div className="bv-empty-state-dashed">
                <h2>No tasks assigned to this story yet.</h2>
                <p>Click '+ Create Task' to break this story down into actionable steps.</p>
              </div>
            )}
          </div>
        </div>
